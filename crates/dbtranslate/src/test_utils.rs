...
impl TestedDialects {
    ...
    pub fn verified_query(&self, sql: &str) -> Query {
        let Statement::Query(query) = self.verified_stmt(sql);
        *query
    }
...
pub fn check_error(sql: &str, expected_error_message: &str, dialect: &dyn Dialect ) {
    let result = Parser::parse_sql(dialect, sql);
    assert!(
        matches!(result, Err(ParserError::ParserError(ref message)) if message == expected_error_message),
        "Expected an error with message '{}', got: {:?}",
