...
// TODO: I believe we want to add Config to the top level query object
// because it should be at the top
...
pub struct Query {
    ...
    /// JinjaVariables - such as {% set variable = "string" %}
    pub jinja_variables: Vec<JinjaVariable>,
}
...
impl fmt::Display for DbtConfig {
...
impl PartialOrd for DbtConfig {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for DbtConfig {
    fn cmp(&self, other: &Self) -> Ordering {
        self.values.len().cmp(&other.values.len())
    }
}

impl Hash for DbtConfig {
...
impl fmt::Display for DbtConfigValue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DbtConfigValue::String(s) => write!(f, "{}", s),
            DbtConfigValue::List(v) => {
                write!(f, "[")?;
                let mut first = true;
                for s in v {
                    if !first {
...
impl fmt::Display for JinjaVariable {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
pub enum JinjaValue {
    Str(String),
    List(Vec<JinjaValue>),
    // Add other value types here as needed, e.g., Function, etc.
}

impl fmt::Display for JinjaValue {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            JinjaValue::Str(s) => write!(f, "{}", s),
            JinjaValue::List(list) => {
                let list_str = list
                    .iter()
